...
$(document).ready(function () {
    $.tools.dateinput.localize("lang", {
        months: mv[0]+','+mv[1]+','+mv[2]+','+mv[3]+','+mv[4]+','+mv[5]+','+mv[6]+','+mv[7]+','+mv[8]+','+mv[9]+','+mv[10]+','+mv[11],//'Januar,Februar,M&auml;rz,April,May,Juni,Juli,August,September,Oktober,November,Dezember',
        shortMonths: mvs[0]+','+mvs[1]+','+mvs[2]+','+mvs[3]+','+mvs[4]+','+mvs[5]+','+mvs[6]+','+mvs[7]+','+mvs[8]+','+mvs[9]+','+mvs[10]+','+mvs[11],
        days: wv[0]+','+wv[1]+','+wv[2]+','+wv[3]+','+wv[4]+','+wv[5]+','+wv[6],
        shortDays: wvs[0]+','+wvs[1]+','+wvs[2]+','+wvs[3]+','+wvs[4]+','+wvs[5]+','+wvs[6]
    });
    $(".date").dateinput({
        lang: 'lang',
        format: 'yyyy-mm-dd',
    ...
    $("body").keyup(function (event) {
        if (event.keyCode == "17") {
            isCtrl = 0;
            if ($("input[name=art]:checked").val() == "UmsatzProdukt") {
                getData();
            }
        }
        return true;
    ...
    $("#seleinheit").click(function () {
        addHistory();
    });
    $(".chaval").change(function () {
        getData()
    });
    ...
    $(".date").each(function () {
        $(this).data("dateinput").onHide(function () {
            getData()
        });
    });
...
{
	if($("input[name=table]:checked").length > 0 || $("#radiochart3:checked").length)
	{
		generateTable();
		$("#table").show();
	}else
...
function addHistory(change) {
    aHistory.push($("#diaform").serializeArray());
...
var title = "";
switch($("input[name=art]:checked").val() )
{
	case "UmsatzProdukt":
		title = aLang["product"];
	break;
	...
	case "UmsatzMonat":
		title = aLang["month"]	;
	break;
	case "UmsatzMarketing":
		title= aLang["marketing"];
	break;
}
	var table = "<table><tr><th>"+title+"</th><th>"+aLang["value"]+"</th><th>"+aLang["description"]+"</th><th>"+aLang["orders"]+"</th><th>"+aLang["soldunits"]+"</th></tr>";
			for(var i = 0; i < allData.length;i++)
			{
				table+="<tr"+((i % 2 == 0)?" class='odd'":"")+">";
...
function changeGraph(func) {
    var type = $('input[name=chart]:checked').val();
    ...
    case "column":
		...
		graph.labelText = "[[value]] " + currency;
		graph.type = type;
		$("#seltabl").removeClass("readonly");
		$("#chart").show();
        break;
    case "table":
    	$("#chart").hide();
    	$("#seltabl").attr("checked",true);
    	$("#seltabl").addClass("readonly");
    	break;
    }
    ...
    if ($("#selbest:checked").val() == "checked") {
        graph2.hidden = false;
    } else {
        graph2.hidden = true;
    }
    if ($("#seleinheit:checked").val() == "checked") {
        graph3.hidden = false;
    } else {
        graph3.hidden = true;
    }
...
function getData() {
    if ($("input[name=art]:checked").val() == "UmsatzProdukt") {
        $("#selvers").attr('checked', false);
        $(".selvars").removeClass("readonly");
        $(".selnetto").removeClass("readonly");
    } else {
        $("#selvers").attr('checked', true);
        $(".selvars").addClass("readonly");
        $(".selnetto").addClass("readonly");
        $("#selnetto").attr("checked", true);
    }
...
function getTotal(arr) {
    ...
    chart.clearLabels();
    chart.addLabel("!20", 5, aLang["shownrevenue"] + format(total) + " " + currency, "right", 12, "black", 0, true);
    if (total == 0) {
        chart.addLabel(150, '50%', aLang["nodata"], "center", 15, "red", 0, false);
    }
}
...
function getMax(arr) {
    ...
    var max2 = 0;
    for (var i = 0; i < arr.length; i++) {
        if (parseFloat(arr[i].value) > max) {
            max = parseFloat(arr[i].value);
        }
        if (parseFloat(arr[i].order) > max2) {
            max2 = parseFloat(arr[i].order);
        }
    }
    valAxis1.minimum = 0;
    valAxis2.minimum = 0;
    valAxis1.maximum = max * 1.1;
    valAxis2.maximum = max2 * 1.1;
}

function deserialize(arr) {
    $("#diaform [type=checkbox]").attr("checked", false);
    for (var i = 0; i < arr.length; i++) {
        fobj = $("#diaform [name=" + arr[i].name + "]");
        if (fobj.length > 1) {
            x = fobj.filter("[value=" + arr[i].value + "]");
            x.attr("checked", true);
        } else if (fobj.length == 1) {
            if (fobj.attr("type") == "checkbox") {
                fobj.attr("checked", true);
            } else if (fobj.attr("type") == "text") {
                fobj.val(arr[i].value);
            } else {
...
function drilldown(objs) {

    var item = objs.item.dataContext;
    check = $("input[name=art]:checked").val()
    if (check == "UmsatzProdukt") {
        var prod = $("input[name=prodfilter]");

        if (isCtrl == 0) {
            $("#radioart2").attr("checked", true);
            $("input[name=prodfilter]").val(item.title);
            $(".selvars").hide();
            $("#selvers").attr('checked', true);
            $("#selnetto").attr("readonly", "readonly");
            $("#selnetto").attr("checked", true);
            getData();
            hasAdded = [];
        } else {
            if (prod.val().search(item.title) >= 0) {
                for (i = 0; i < hasAdded.length; i++) {
                    if (hasAdded[i] == item.title) {
                        hasAdded.splice(i, 1);
                    }
                }
            } else {
                hasAdded.push(item.title);
            }
            prod.val("");
            for (i = 0; i < hasAdded.length - 1; i++) {
                prod.val(prod.val() + hasAdded[i] + ";");
            }
            if (hasAdded.length > 0) {
                prod.val(prod.val() + hasAdded[hasAdded.length - 1]);
            }

        }
    } else if (check == "UmsatzMonat") {
        start = $("#start").data("dateinput");
        end = $("#end").data("dateinput");
        dates = new Date(parseInt(item.title.split(" ")[1]), parseInt(item.month), 1);
        start.setValue(dates);
        end.setValue(dates).addMonth(1).addDay(-1);
        $("#radioart1").attr("checked", true);
        getData();
    } else if (check == "UmsatzDatum") {
        start = $("#start").data("dateinput");
        end = $("#end").data("dateinput");
        dates = new Date(parseInt(item.title.split(".")[2]), parseInt(item.title.split(".")[1]) - 1, parseInt(item.title.split(".")[0]));
        start.setValue(dates);
        end.setValue(dates);
        $("#radioart3").attr("checked", true);
        getData();
    }

}

function generateDia(chartData) {
    chart = new AmCharts.AmSerialChart();
    chart.dataProvider = chartData;
    chart.categoryField = "title";
    chart.pathToImages = resourceurl + "amcharts/javascript/images/";
    chart.marginTop = 25;
    ...
    chart.rotate = true;
    chart.zoomToIndexes(0, 20);
    chart.addListener("clickGraphItem", drilldown)

    valAxis1 = new AmCharts.ValueAxis();
    valAxis1.axisColor = "#004655";
    ...
    graph.balloonText = "[[description]]\nBetrag: [[value]] " + currency;
    graph.title = "Gesamtumsatz";
    graph.lineAlpha = 1;
    graph.fillAlphas = 1;
    graph.bullet = "round";
    graph.color = "#004655";
    graph.fillColors = "#004655";
    graph.lineColor = "#646464";
    graph.labelPosition = "top";



    valAxis2 = new AmCharts.ValueAxis();
    valAxis2.position = "left";
    valAxis2.axisColor = "#dddddd";
    ...
    graph2.valueField = "order";
    graph2.valueAxis = valAxis2;
    graph2.labelText = "";
    graph2.balloonText = "Bestellungen [[value]]";
    graph2.title = "Bestellungen";
    graph2.lineAlpha = 1;
    graph2.fillAlphas = 0;
    graph2.lineColor = "#aaaaaa";
    graph2.bullet = "round";
    graph2.hidden = true;


    graph3 = new AmCharts.AmGraph();
    graph3.valueField = "sold";
    graph3.valueAxis = valAxis2;
    graph3.labelText = "";
    graph3.balloonText = "Verkaufe Einheiten [[value]]";
    graph3.title = "Bestellungen";
    graph3.lineAlpha = 0.8;
    graph3.fillAlphas = 0;
    graph3.lineColor = "#2b92aa";
    graph3.bullet = "round";
    graph3.hidden = true;
    ...
    chartScrollbar.selectedBackgroundColor = "#646464";
    chartScrollbar.selectedGraphFillColor = "#FFFFFF";
    chart.addChartScrollbar(chartScrollbar);

    var trendLine = new AmCharts.TrendLine();
    trendLine.initialCategory = "0";
    trendLine.finalCategory = "21";
    trendLine.initialValue = 1000;
