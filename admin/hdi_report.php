...
//      ini_set('display_errors', true);
	...
	protected function getTrend(&$trend, $avg, $i)
	{
		...
		if($this->art == "UmsatzDatum")
		{
			...
			foreach($array as $item)
			{
				if($this->sort == "ohne")
				{
					...
					if(oxConfig::getParameter("aufsteigend"))
					{
						while( $lDate < strtotime($item[0]) )
						{

							$b[strftime("%Y-%m-%d",$lDate)] = array(strftime('%d.%m.%Y',$lDate), 0,"",0,0,"");
							//echo "lDate:".$lDate." - aDate:".(strtotime($item[0])- (60*60*24))."\n";
							//echo "Hinzugefügt:".$lDate." - ".strftime("%Y-%m-%d",$lDate)."\n";
							$lDate+=(60*60*24);
						}
					}else{
						while( $lDate > strtotime($item[0]) )
				...
				//echo "Übersprungen:".$lDate." - ".$item[0]."\n";
				$lDate = strtotime($item[0])+(60*60*24);
			}
			$array = $b;
		}elseif($this->art == "UmsatzMonat")
		{
			$b= array();
			$oLang = oxLang::getInstance();
			$month = json_decode($oLang->translateString("HDIREPORT_MONTHVALUES"));
